...
export function scoreTone(score: number): { text: string; stroke: string } {
  ...
  if (score >= 50) return { text: "text-yellow-600", stroke: "stroke-yellow-500" };
...
function TextSection({ title, body }: { title: string; body: string | null }) {
  return (
    <section>
      ...
      {body ? (
        <p className="mt-1 whitespace-pre-line text-sm leading-6 text-gray-700">{body}</p>
      ) : (
      ...
      {items && items.length > 0 ? (
        <ul className="mt-2 space-y-1.5">
          {items.map((item, index) => (
            <li key={`${index}-${item}`} className="flex gap-2 text-sm text-gray-700">
              <span
                aria-hidden="true"
        ...
        <p className="mt-1 text-sm italic text-gray-400">None identified</p>
      )}
    </section>
  );
}
...
function InterviewChip({ value }: { value: boolean | null }) {
  if (value === null) {
    return (
      <span className="inline-flex items-center rounded-full bg-gray-100 px-3 py-1 text-sm font-medium text-gray-600 ring-1 ring-inset ring-gray-500/20">
        Not specified
      </span>
    );
  }
  return value ? (
    <span className="inline-flex items-center gap-1.5 rounded-full bg-green-100 px-3 py-1 text-sm font-medium text-green-800 ring-1 ring-inset ring-green-600/20">
      <span aria-hidden="true">✓</span> Yes
    </span>
  ) : (
    <span className="inline-flex items-center gap-1.5 rounded-full bg-gray-100 px-3 py-1 text-sm font-medium text-gray-700 ring-1 ring-inset ring-gray-500/20">
...
export function ResultCard({ result }: { result: ScreeningResult }) {
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
      <div className="flex flex-col gap-6 border-b border-gray-200 p-6 sm:flex-row sm:items-center">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-3 text-sm leading-6 text-gray-700">{result.decision_reason}</p>
        </div>
      </div>
      ...
      <div className="grid gap-6 p-6 md:grid-cols-2">
        ...
        <ListSection title="Missing Skills" items={result.missing_skills} bullet="bg-red-400" />
        <ListSection title="Strengths" items={result.strengths} bullet="bg-green-500" />
        <ListSection title="Concerns" items={result.concerns} bullet="bg-yellow-500" />
      </div>
